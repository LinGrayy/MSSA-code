...
class FewShotSeg(nn.Module):
    ...
    def get_encoder(self):
        ...
        if self.config['which_model'] == 'dlfcn_res101' or self.config['which_model'] == 'default':
            ...
            self.encoder = torch.hub.load(
                'facebookresearch/dinov2', 'dinov2_vitl14') # dinov3_vitl16   dinov2_vitl14
        ...
        elif self.config['which_model'] == 'medclip':
            # 添加 MedCLIP 模型加载代码
            try:
                ...
                # 或者使用 MedCLIPModel.from_pretrained()
            except ImportError:
                raise ImportError("MedCLIP package is not installed. Please install it first.")
            # 设置特征图尺寸，MedCLIP 通常使用 ViT 架构，patch size 一般为 16
            self.config['feature_hw'] = [max(
                self.image_size//16, DEFAULT_FEATURE_SIZE), max(self.image_size//16, DEFAULT_FEATURE_SIZE)]
            ...
            self.config['feature_hw'] = [max(
                self.image_size//14, DEFAULT_FEATURE_SIZE), max(self.image_size//14, DEFAULT_FEATURE_SIZE)]
        elif self.config['which_model'] == 'dinov3_l16':
            self.encoder = torch.hub.load(
            ...
            # Segment Anything Model - ViT-Huge
            from segment_anything import sam_model_registry
            sam = sam_model_registry["vit_h"](checkpoint="/home/psxll9/MedCLIP-SAM/checkpoint/sam_vit_h_4b8939.pth")
    ...
    def get_features(self, imgs_concat):
        ...
        if 'dinov3' in self.config['which_model']:
        ...
        elif 'sam' in self.config['which_model']:
            # SAM模型处理
            target_size = (self.config.get('sam_input_size', 1024), 
                   self.config.get('sam_input_size', 1024))
    
            # 调整输入图像尺寸
            imgs_concat = F.interpolate(imgs_concat, size=target_size, mode='bilinear')
            ...
            if H < DEFAULT_FEATURE_SIZE or W < DEFAULT_FEATURE_SIZE:
                img_fts = F.interpolate(img_fts, size=(
                    max(H, DEFAULT_FEATURE_SIZE), max(W, DEFAULT_FEATURE_SIZE)), mode='bilinear')
            ...
            if HW < DEFAULT_FEATURE_SIZE ** 2:
                img_fts = F.interpolate(img_fts, size=(
                    DEFAULT_FEATURE_SIZE, DEFAULT_FEATURE_SIZE), mode='bilinear')  # this is if h,w < (32,32)
        elif self.config['which_model'] == 'medclip':  # 或者 'BiomedCLIP'，根据您的配置
            # BiomedCLIP 特征提取
            with torch.no_grad():
                # 预处理图像（如果需要）
                imgs_concat = (imgs_concat - imgs_concat.min()) / (imgs_concat.max() - imgs_concat.min())
                # imgs_concat = F.interpolate(imgs_concat, size=(
                # self.image_size // 16 * 16, self.image_size // 16 * 16), mode='bilinear')
                if hasattr(self, 'processor'):
                    # 使用处理器预处理图像
                    inputs = self.processor(images=imgs_concat, return_tensors="pt")
                    inputs = {k: v.to(imgs_concat.device) for k, v in inputs.items()}
                    
                    # 提取视觉特征
                    outputs = self.encoder.vision_model(**inputs)
                    img_fts = outputs.last_hidden_state  # B, seq_len, hidden_size
                    
                    # 移除CLS token，只保留patch tokens
                    img_fts = img_fts[:, 1:, :]  # B, HW, C
                    
                else:
                    # 如果没有processor，直接使用模型
                    outputs = self.encoder.vision_model(pixel_values=imgs_concat)
                    img_fts = outputs.last_hidden_state[:, 1:, :]  # B, HW, C
                
                # 重新排列维度以匹配其他模型的特征格式
                img_fts = img_fts.permute(0, 2, 1)  # B, C, HW
                B, C, HW = img_fts.shape
                
                # 重塑为空间特征图
                H = W = int(HW ** 0.5)
                img_fts = img_fts.view(B, C, H, W)
                
                # 如果需要，进行插值
                if H < DEFAULT_FEATURE_SIZE:
    ...
    def get_cls(self):
        ...
        if self.config['cls_name'] == 'grid_proto':
            ...
            if 'dinov2_b14' in self.config['which_model']:
                ...
                embed_dim = 1024
            elif 'dinov3_vitl16' or 'sam' in self.config['which_model']:
                embed_dim = 1024
            elif self.config['which_model'] == 'medclip':
                # BiomedCLIP 使用 ViT-B/16，特征维度为768
                embed_dim = 768
